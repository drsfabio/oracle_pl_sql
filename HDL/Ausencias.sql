...
FROM
  ...
  INNER JOIN per_periods_of_service periodsofservice ON (
    periodsofservice.period_of_service_id = absenceentries.period_of_service_id
  )
  INNER JOIN per_all_people_f allpeople ON (
    allpeople.person_id = periodsofservice.person_id
  )
  AND (
    trunc (sysdate) BETWEEN allpeople.effective_start_date
    AND allpeople.effective_end_date
  )
  INNER JOIN anc_absence_types_vl absencetypes ON (
    absencetypes.absence_type_id = absenceentries.absence_type_id
  )
  AND (
    trunc (sysdate) BETWEEN absencetypes.effective_start_date
    AND absencetypes.effective_end_date
  )
  INNER JOIN per_all_assignments_m allassignments ON (
