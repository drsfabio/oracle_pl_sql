SELECT '1' AS "Tipo",
       '' AS "Unidade Marítima",
       asg.ass_attribute1 AS "CNPJ/CPF/CEI/CAEPF",
  ...
  ( SELECT GEO.GEOGRAPHY_NAME
   ...
   WHERE GEO.GEOGRAPHY_CODE = RG.ATTRIBUTE1
     ...
     AND ROWNUM = 1 ) AS "UF Emissão RG",
       ...
       '' AS "RNE",
       substr(asg.assignment_number, 4, 100) AS "Matrícula",
       '' AS "Matrícula RH",
       '2' AS "Vínculo",
       'NA' AS "BR/PDH",
       'NA' AS "Reg. Revez",
  ...
  ( SELECT POS.POSITION_CODE
   ...
   WHERE POS.POSITION_ID = ASG.POSITION_ID
     AND ASG.EFFECTIVE_END_DATE BETWEEN POS.EFFECTIVE_START_DATE AND POS.EFFECTIVE_END_DATE ) AS "Código Posição Trabalho",

  ( SELECT POS.NAME
   FROM HR_ALL_POSITIONS_F_VL POS
   WHERE POS.POSITION_ID = ASG.POSITION_ID
     AND ASG.EFFECTIVE_END_DATE BETWEEN POS.EFFECTIVE_START_DATE AND POS.EFFECTIVE_END_DATE ) AS "Posição Trabalho",
